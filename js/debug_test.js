...
// Mocks
const document = {
    getElementById: () => ({ innerHTML: '', appendChild: () => {}, classList: { add:()=>{}, remove:()=>{} }, querySelectorAll: () => [] }),
    querySelectorAll: () => [],
...
setTimeout(async () => {
    try {
        ...
        console.log('Players count:', gm.players.length);
        gm.players.forEach(p => {
            console.log('Player ' + p.name + ' Hand: ' + p.hand.length);
             if (p.hand.length > 0) {
                console.log('  Sample card Strength:', p.hand[0].strength);
            }
        });
        console.log('Deck remaining:', gm.deck.cards.length);
    } catch (e) {
